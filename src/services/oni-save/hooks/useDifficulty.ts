...
export interface UseDifficulty {
  difficulty: Record<keyof typeof QualityLevelSettingValues, string>;
  onModifyDifficulty(
    type: keyof typeof QualityLevelSettingValues,
    value: string,
  ): void;
}
...
export default function useDifficulty(): UseDifficulty {
  ...
  const difficulty: Record<keyof typeof QualityLevelSettingValues, string> =
    {} as any;
  ...
  const onModifyDifficulty = React.useCallback(
    (type: keyof typeof QualityLevelSettingValues, value: string) => {
